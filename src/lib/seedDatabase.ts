...
const seedOpportunities = [
  ...
  },
  {
    ...
    full_description: 'The Young Global Leaders community is a unique, multi-stakeholder community of more than 1,400 members and alumni of exceptional leaders under 40 who are driving change across business, government, and civil society. Selected through a rigorous nomination and selection process, YGLs participate in a five-year program of leadership development, peer learning, and collaborative action. Members gain access to World Economic Forum meetings, including the Annual Meeting in Davos, and work together on initiatives addressing global challenges from climate change to inclusive growth.',
    deadline: '2024-07-15',
  ...
  },
  {
    ...
    full_description: 'Meta University is a paid 10-week summer internship program designed to provide underrepresented groups, including first-generation college students, with an introduction to careers in technology. Interns work on meaningful projects across engineering, analytics, and design, while receiving mentorship and professional development support. The program includes technical training, exposure to Meta\'s products and culture, and networking opportunities with leaders across the company. Meta University alumni often return for additional internships and full-time positions at Meta.',
    deadline: '2024-11-15',
  ...
  },
  {
    ...
    full_description: 'Microsoft Imagine Cup is a global technology competition that empowers the next generation of computer science students to apply their passion and creativity to solve real-world problems. Teams create innovative solutions using Microsoft technologies including Azure, AI, and mixed reality. The competition includes regional rounds, online semifinals, and a World Finals where top teams compete for prizes, mentorship, and the opportunity to bring their ideas to life. Past participants have gone on to launch successful startups and careers at leading tech companies.',
    deadline: '2024-02-15',
    location: 'Global',
    amount: '$100,000',
...
export async function seedDatabase() {
  try {
    // Check if database already has opportunities
    ...
    for (const opp of seedOpportunities) {
      ...
      if (oppError) {
        console.error(`Error inserting opportunity: ${opp.title}`, oppError);
        continue;
      }

      // Insert tags
      if (opp.tags && opp.tags.length > 0) {
        for (const tagName of opp.tags) {
          ...
          const { error: linkError } = await supabase.from('opportunity_tags').insert({
            opportunity_id: opportunity.id,
            tag_id: tag.id,
          });

          if (linkError) {
            console.error(`Error linking tag to opportunity`, linkError);
          }
        }
      }
    }
